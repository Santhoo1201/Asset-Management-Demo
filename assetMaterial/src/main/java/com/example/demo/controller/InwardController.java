...
public class InwardController {
	...
	public ResponseEntity<List<DulpicateMaterialDetails>> getAllInwardDetails(){
		...
		for(InwardDetails toSetValue : getTheInwardDetails) {
			...
			String getVisitorName = getSplitVisitors[0];
			String getVisitorNo = getSplitVisitors[1];
			settedInwardDetails.add(new DulpicateMaterialDetails(getAssetNo,getMaterialName,getModelName,getVisitorName,toSetValue.getReceiverDate(),toSetValue.getReceiverSfmBy()));
