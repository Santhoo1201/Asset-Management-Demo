...
//	}
	...
	public ResponseEntity<List<DulpicateMaterialDetails>> getAllOutwardDetails(){
		...
		for(OutwardDetails setValue:getValue) {
			...
			if(getAssetMaterialNameModel.length==3 && getAssignedName.length==3 ) {
				...
				String personName="";
				if(setValue.getVendor() == null  || setValue.getVendor().isEmpty()) {
					String[] getIdNamerole=setValue.getReceiverSfm().split("--");
					 personName =getIdNamerole[1];
				}else {
					
					String []getVendorsAndNumber = setValue.getVendor().split("--");
					personName = getVendorsAndNumber[0];
				}
